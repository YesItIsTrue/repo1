...
ASSIGN  errlog1 = "C:\PROGRESS\WRK\ISO-CleanUP-addr_mstr-country-" + UPPER(STRING(update-flag)) + ".txt".   
...
ASSIGN  kount-desc [2]  = ":Total country_mstr records not found.".
ASSIGN  kount-desc [3]  = ":Total addr_mstr records flagged for deletion.". 
ASSIGN  kount-desc [4]  = ":Total addr_mstr records updated   with new ISO country.".
...
ASSIGN  kount-desc [7]  = ":Total addr_mstr records with BLANK addr_country-NOT updated.".
...
/* Found addr_country in country_mstr - check to see it a address record 
    does or does not exist with the new country_ISO. */         
        IF o-fcountry-error =  NO THEN  DO:       /* logical NO = data is found */
        
            ASSIGN DIFF-addr-Id = o-fcountry-error.
...
/* addr_mstr does not exist with new country_ISO. Update the addr_country field only.*/
            IF NOT AVAILABLE (h-address-buf) THEN DO: 
                ...
                IF update-flag = YES THEN 
                    ASSIGN General.addr_mstr.addr_country          = o-fcountry-ISO
                           General.addr_mstr.addr_modified_by      = USERID ("General")
                           General.addr_mstr.addr_modified_date    = TODAY
                           General.addr_mstr.addr_prog_name        = "ISO-CleanUP-addr_mstr-country.p - addr_mstr.addr_country updated with new country_ISO.".
                                           
                ASSIGN  kount [4] = (kount [4] + 1).    /* addr_mstr updated with new ISO country. */
...
/* UPDATE the people_mstr WITH the NEW address-ID . */                   
                        ...
                        IF update-flag = YES THEN 
                            ASSIGN  
                                ...
                                General.people_mstr.people_prog_name        = "ISO-CleanUP-addr_mstr-country.p - people_addr_id new address-ID.".
...
/* UPDATE the people_mstr WITH the NEW address-ID . */                   
                        ...
                        IF update-flag = YES THEN
                            ASSIGN  
                                ...
                                General.people_mstr.people_modified_by      = USERID ("General")
                                General.people_mstr.people_modified_date    = TODAY
                                General.people_mstr.people_prog_name        = "ISO-CleanUP-addr_mstr-country.p - people_second_addr_id new address-ID.".
