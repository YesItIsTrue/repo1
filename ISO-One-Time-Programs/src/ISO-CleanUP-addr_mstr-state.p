...
DEFINE BUFFER   h-address-buf  FOR General.addr_mstr. 
...
DEFINE VARIABLE i-country    LIKE General.state_mstr.state_country_ISO          NO-UNDO. 
DEFINE VARIABLE i-state      LIKE General.state_mstr.state_display_name         NO-UNDO.  
DEFINE VARIABLE o-fstate-ISO     LIKE General.country_mstr.Country_ISO          NO-UNDO.
...
/*DEFINE VARIABLE i-country-display-name     LIKE General.country_mstr.Country_Display_Name  NO-UNDO.*/
...
ASSIGN  xx_prog_name = SUBSTRING(THIS-PROCEDURE:FILE-NAME,R-INDEX(THIS-PROCEDURE:FILE-NAME,"\") + 1).
...
ASSIGN  errlog1 = "C:\PROGRESS\WRK\ISO-CleanUP-addr_mstr-state-" + UPPER(STRING(update-flag)) + ".txt".   
...
/* ***************************  Main Block  *************************** */
ASSIGN  kount-desc [1]  = ":Total input addr_mstr records processed.".
ASSIGN  kount-desc [2]  = ":Total state_mstr records not found.".
ASSIGN  kount-desc [3]  = ":Total addr_mstr records flagged for deletion.". 
ASSIGN  kount-desc [4]  = ":Total addr_mstr records updated   with new ISO state.".
ASSIGN  kount-desc [5]  = ":Total people_mstr records people_addr_id updated with new address-ID.".               
ASSIGN  kount-desc [6]  = ":Total people_mstr records people_second_addr_id updated with new address-ID.".
ASSIGN  kount-desc [7]  = ":Total addr_mstr records with BLANK addr_stateprov-NOT updated.".

main-loop:
FOR EACH General.addr_mstr  
        WHERE  General.addr_mstr.addr_deleted = ? 
    EXCLUSIVE-LOCK BY addr_id:
        
        ASSIGN  h-addr_ID   = General.addr_mstr.addr_id
                h-addr1     = General.addr_mstr.addr_addr1
                h-addr2     = General.addr_mstr.addr_addr2
                h-addr3     = General.addr_mstr.addr_addr3
                h-city      = General.addr_mstr.addr_city
                h-stateprov = General.addr_mstr.addr_stateprov  
                h-zip       = General.addr_mstr.addr_zip
                h-country   = General.addr_mstr.addr_country.
                
        ASSIGN  kount [1] = (kount [1] + 1).
        
        ASSIGN  o-fstate-ISO          = "Not Found"
                o-fstate-error        = YES.

        IF  h-stateprov = "" THEN DO:
            EXPORT STREAM outward1 DELIMITER ";" 
                         h-addr_ID
                         h-addr1
                         h-addr2
            ...
            ASSIGN  kount [7] = (kount [7] + 1).
            NEXT main-loop.
        END. /* IF h-stateprov = ""  */
        
        ASSIGN  i-country = h-country
                i-state   = h-stateprov.
...
/* Found addr_stateprov in state_mstr - check to see it a address record 
    does or does not exist with the new state_ISO. */         
        IF o-fstate-error =  NO THEN  DO:       /* logical NO = data is found */
        
            ASSIGN DIFF-addr-Id = NO.
...
/* addr_mstr does not exist with new state_ISO. Update the addr_stateprov field only.*/
            IF NOT AVAILABLE (h-address-buf) THEN DO: 
                
                IF update-flag = YES THEN 
                    ASSIGN General.addr_mstr.addr_stateprov        = o-fstate-ISO
                           General.addr_mstr.addr_modified_by      = USERID ("General")
                           General.addr_mstr.addr_modified_date    = TODAY
                           General.addr_mstr.addr_prog_name        = "ISO-CleanUP-addr_mstr-state.p - addr_mstr.addr_stateprov updated with new state_ISO.".
                                           
                ASSIGN  kount [4] = (kount [4] + 1).    /* addr_mstr updated with new ISO state. */
...
*/   
            IF  AVAILABLE (h-address-buf) THEN DO: 
                
                FOR EACH  General.people_mstr WHERE 
                                General.people_mstr.people_addr_id = General.addr_mstr.addr_id  AND
                                General.people_mstr.people_deleted = ?
                        EXCLUSIVE-LOCK:

                    IF  General.people_mstr.people_addr_id <> h-address-buf.addr_id THEN DO: 
/* UPDATE the people_mstr WITH the NEW address-ID . */                   
                        ASSIGN  kount [5] = (kount [5] + 1).    /* General.people_mstr records people_addr_id updated with new address-ID. */                         
                        ASSIGN DIFF-addr-Id = YES.  
                            
                        IF update-flag = YES THEN 
                            ASSIGN  
                                General.people_mstr.people_addr_id          = h-address-buf.addr_id
                                General.people_mstr.people_modified_by      = USERID ("General")
                                General.people_mstr.people_modified_date    = TODAY
                                General.people_mstr.people_prog_name        = "ISO-CleanUP-addr_mstr-state.p - people_addr_id new address-ID.".
                                
                    END.  /* IF  General.people_mstr.people_addr_id <> h-address-buf.addr_id  */
                     
                END.  /*  FOR EACH  General.people_mstr.people_addr_id  */
                
/*  look in the second addr_id field. */
                FOR EACH  General.people_mstr WHERE 
                                General.people_mstr.people_second_addr_ID   = General.addr_mstr.addr_id   AND
                                General.people_mstr.people_deleted = ?
                        EXCLUSIVE-LOCK:

                    IF  General.people_mstr.people_second_addr_id <> h-address-buf.addr_id THEN DO:
/* UPDATE the people_mstr WITH the NEW address-ID . */                   
                        ASSIGN  kount [6] = (kount [6] + 1).    /* General.people_mstr records people_second_addr_id updated with new address-ID */
                        ASSIGN DIFF-addr-Id = YES.
                        
                        IF update-flag = YES THEN
                            ASSIGN  
                                General.people_mstr.people_second_addr_ID   = h-address-buf.addr_id
                                General.people_mstr.people_modified_by      = USERID ("General")
                                General.people_mstr.people_modified_date    = TODAY
                                General.people_mstr.people_prog_name        = "ISO-CleanUP-addr_mstr-state.p - people_second_addr_id new address-ID.".

                    END.  /* IF  General.people_mstr.people_second_addr_id <> h-address-buf.addr_id */
                    
                END.  /*  FOR EACH  General.people_mstr.people_second_addr_ID   */
