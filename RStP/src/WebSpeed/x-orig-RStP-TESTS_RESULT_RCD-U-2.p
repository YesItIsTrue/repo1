...
DEFINE VARIABLE firstrun AS LOGICAL INITIAL YES    NO-UNDO.       /* change to YES for number 2 program. */

DEFINE VARIABLE loadedlist      AS CHARACTER 
    INITIAL "DL,AL,UL,IL" NO-UNDO.
DEFINE VARIABLE recordsprocessed AS INTEGER NO-UNDO.
    
DEFINE VARIABLE ERROR-count AS INTEGER EXTENT 9 NO-UNDO.                        /* 1dot4 */
...
DEFINE VARIABLE errlog AS CHARACTER 
    INITIAL "C:\PROGRESS\WRK\RStP-TESTS_RESULT_RCD-U-log-2.txt" NO-UNDO.        /* 1dot4 */ /* change to 2 for number 2 program */
...
/*    IF TESTS_RESULT_RCD.Progress_Flag = "" AND firstrun = NO THEN               /* 1dot4 */           /* 1dot7 */*/
/*        ASSIGN  TESTS_RESULT_RCD.Progress_Flag = "U".                           /* 1dot4 */           /* 1dot7 */*/
    ...
    IF tests_result_rcd.test_kit_nbr < "A" OR 
        tests_result_rcd.test_abbv = "CPR" THEN DO: 
        
        /* 1dot9 */
        FIND FIRST TK_mstr WHERE TK_mstr.TK_patient_ID = INTEGER(TESTS_RESULT_RCD.PatientID) AND            /* 1dot4 */
                TK_mstr.TK_lab_sample_ID = TESTS_RESULT_RCD.Lab_Sampleid AND                                /* 1dot4 */
                TK_mstr.TK_lab_seq =  INTEGER(TESTS_RESULT_RCD.Lab_Sampleid_SeqNbr) AND                     /* 1dot4 */
                TK_mstr.TK_test_type = TESTS_RESULT_RCD.Test_Abbv  AND                   /* 2dot0 */       /* 1dot4 */
                TK_mstr.TK_ID = TESTS_RESULT_RCD.Test_Kit_Nbr                                               /* 1dot4 */
                    USE-INDEX TK-old-idx                                                 /* 2dot0 */      /* 1dot4 */
                    NO-LOCK NO-ERROR.                                                                       /* 1dot4 */
        /* 1dot9 */

        IF NOT AVAILABLE (tk_mstr) THEN DO: 
        
            FIND LAST tk_mstr WHERE tk_mstr.tk_id = tests_result_rcd.test_kit_nbr
                NO-LOCK NO-ERROR.
                
            IF AVAILABLE (tk_mstr) THEN 
                v-seqNbr = tk_mstr.tk_test_seq + 1.
                
            ELSE 
                v-seqNbr = 1.
                
        END.  /** of if not avail tk_mstr --- the first time **/
         
        ELSE                                                                                                    /* 2dot1 */
            v-seqnbr = INTEGER(TESTS_RESULT_RCD.Lab_Sampleid_SeqNbr).                                           /* 2dot1 */         

    END.  /** of if tests_result_rcd.test_kit_nbr < "A" or test_abbv = "CPR" **/
    
    ELSE DO: 
    
        FIND TK_mstr WHERE  TK_mstr.TK_ID = TESTS_RESULT_RCD.Test_Kit_Nbr AND                                   /* 1dot9 */
                            TK_mstr.TK_test_seq = INTEGER(TESTS_RESULT_RCD.Lab_Sampleid_SeqNbr) AND             /* 1dot9 */
                            TK_mstr.TK_test_type = TESTS_RESULT_RCD.Test_Abbv AND                               /* 2dot1 */
                            TK_mstr.TK_deleted = ? NO-LOCK NO-ERROR.                                            /* 1dot9 */
    ...
    END.  /** of else do -- test_kit_nbr >= "A" and not CPR --- modern data **/
...
/*  Set up the next TK test seq number to use.  */                              /* 1dot4 */
    IF NOT AVAILABLE (TK_mstr) THEN                                         /* 1dot4 */ 
/*        ASSIGN v-seqNbr = 0.                                                    /* 1dot4 */*/     /* 1dot8 */
        ASSIGN v-seqNbr = 1.                                                    /* 1dot8 */ 
    ELSE                                                                        /* 1dot4 */ 
        ASSIGN v-seqNbr = TK_mstr.TK_test_seq.                                  /* 1dot4 */ 
**********/

    /*** End of 2dot0 ***/

 
/* Now start to process the input data.  */ 
...
/*        ASSIGN v-seqNbr = v-seqNbr + 1.                                         /* 1dot3 */*/     /* 1dot8 */
   
        FIND TK_mstr WHERE TK_mstr.TK_ID = Tests_Result_RCD.Test_Kit_nbr AND   
            TK_mstr.TK_test_seq = v-seqNbr AND 
            TK_mstr.TK_test_type = TESTS_RESULT_RCD.Test_Abbv AND                                           /* 2dot1 */
            TK_mstr.TK_deleted = ?
                NO-LOCK NO-ERROR. 
        ...
        IF AVAILABLE (tk_mstr) THEN DO:  
            ...
            EXPORT STREAM outward DELIMITER ";"
               ...
               "ERROR 1! TK_mstr.TK_ID or TK_test_seq already exists.  Add NOT done!".
            ASSIGN ERROR-count[1] = ERROR-count[1]  + 1.                        /* 1dot4 */       
                
        END.  /** AVAILABLE (tk_mstr), already exists **/
        
        ELSE DO:        /** normal condition for a blank or A record --- Add the record **/
        
            FIND Lab_mstr WHERE Lab_mstr.Lab_ID = TESTS_RESULT_RCD.Test_LAB_Name NO-LOCK NO-ERROR.  
            ...
            IF it-message = YES THEN 
                DISPLAY "before run" Tests_Result_RCD.test_kit_nbr FORMAT "x(12)" Tests_Result_RCD.Lab_SampleID_seqNbr.
            ...
            ELSE DO:
                ...
                FIND TK_mstr WHERE TK_mstr.TK_ID = v-testID AND 
                    TK_mstr.TK_test_seq = v-seqNbr AND 
                    TK_mstr.TK_test_type = TESTS_RESULT_RCD.Test_Abbv                                   /* 2dot1 */
...
{RStP-TESTS-trh-TK-Status-Date-U.i}.                                             /* 1dot6 */  
        ...
        END.  /** of else do --- not avail tk_mstr - normal condition - add the record **/
...
/*        IF  v-seqNbr = 0 THEN                                                   /* 1dot4 */*/         /* 1dot8 */
/*            v-seqNbr = 1.                                                       /* 1dot4 */*/         /* 1dot8 */
        ...
        IF NOT AVAILABLE (tk_mstr) THEN DO:  
            ...
            EXPORT STREAM outward DELIMITER ";"
                ...
                v-seqNbr
                Tests_Result_RCD.Progress_Flag                                  /* 1dot4 */ 
                "ERROR 5! Tests_Result record not found to delete from database.".
            ASSIGN Tests_Result_RCD.Progress_Flag = "DL".                       /** stands for Delete Loaded **/
            ASSIGN ERROR-count[5] = ERROR-count[5]  + 1.                        /* 1dot4 */    
        END.  /** NOT AVAILABLE (tk_mstr) **/
        
        ELSE DO:        /** normal condition for a D record --- Delete the record **/
     
            IF it-message = YES THEN 
                DISPLAY "test_seq" TK_mstr.TK_test_seq.
        ...
        END.  /*** of else do --- normal condition for a D record --- Delete the record ***/
...
/*            v-seqNbr = 1.                                                       /* 1dot4 */*/         /* 1dot8 */
        ...
        IF NOT AVAILABLE (TK_mstr) THEN DO:  
            ...
            EXPORT STREAM outward DELIMITER ";"
                ...
                v-seqNbr
                Tests_Result_RCD.Progress_Flag                                  /* 1dot4 */ 
                "ERROR 6! TK_mstr record not found to Update.".
...
{RStP-TESTS-trh-TK-Status-Date-U.i}.                                             /* 1dot6 */
        ...
        END. /* ELSE DO - avail TK_mstr - normal update condition */          
