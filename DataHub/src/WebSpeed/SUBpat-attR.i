...
DEFINE VARIABLE v-filechecker   AS CHARACTER NO-UNDO. 
...
/* ***************************  Main Block  *************************** */
    ...
    FOR EACH att_files WHERE
        ...
        ASSIGN v-filechecker = "".

        IF SUBSTRING(att_files.att_filepath,1,4) = "http" THEN
            ASSIGN v-filechecker = att_files.att_filepath + att_files.att_filename.
    
        ELSE IF SUBSTRING(att_files.att_filename,1,4) = "http" THEN
            ASSIGN v-filechecker = att_files.att_filename + att_files.att_filepath.

        ELSE IF SUBSTRING(att_files.att_filepath,1,4) = "file" THEN 
            ASSIGN v-filechecker = "http://HHI-DC-4.hhi.local" + SUBSTRING(att_files.att_filepath,26) + att_files.att_filename.
            
        ELSE IF SUBSTRING(att_files.att_filepath,1,7) = "C:\DATA" THEN 
            ASSIGN v-filechecker = "http://HHI-DC-4.hhi.local" + SUBSTRING(att_files.att_filepath,8) + att_files.att_filename.
        
        ELSE IF SUBSTRING(att_files.att_filepath,1,7) = "C:\APPS" THEN
        ...
        ELSE IF SUBSTRING(att_files.att_filename,1,4) = "file" THEN 
            ASSIGN v-filechecker = "http://HHI-DC-4.hhi.local" + SUBSTRING(att_files.att_filename,26) + att_files.att_filepath.
            
        ELSE IF SUBSTRING(att_files.att_filename,1,7) = "C:\DATA" THEN 
            ASSIGN v-filechecker = "http://HHI-DC-4.hhi.local" + SUBSTRING(att_files.att_filename,8) + att_files.att_filepath.
            
        ELSE IF SUBSTRING(att_files.att_filename,1,7) = "C:\APPS" THEN
    ...
    FOR EACH TK_mstr WHERE
        ...
        IF FIRST-OF(TK_mstr.tk_id) THEN 
            {&OUT}
                "       <tr>" SKIP
                "           <td nowrap >" att_files.att_category "</td>" SKIP
