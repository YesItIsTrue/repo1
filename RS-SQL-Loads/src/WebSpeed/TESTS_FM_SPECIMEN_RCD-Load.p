...
/*------------------------------------------------------------------------
    File        : TESTS_FM_SPECIMEN_RCD-Load.p
    Purpose     : Load the TESTS_FM_SPECIMEN_RCD from SQL. 
...
DEFINE TEMP-TABLE tt LIKE TESTS_FM_SPECIMEN_RCD.
...
DEFINE VARIABLE loadlog AS CHARACTER 
    INITIAL "C:\PROGRESS\WRK\SQL-Load-TESTS_FM_SPECIMEN_RCD-log.txt" NO-UNDO.
...
INPUT FROM VALUE(SEARCH("Input-Files\TESTS_FM_SPECIMEN_RCD_NONULLS.txt")).               /* 1dot3 */

/*IF drive_letter = "P" THEN                                                                        */
/*    INPUT FROM "P:\OpenEdge\WRK\RS-SQL-Loads\Input-Files\TESTS_FM_SPECIMEN_RCD_NONULLS.txt".      */
/*ELSE                                                                                              */
/*    INPUT FROM "C:\OpenEdge\Workspace\RS-SQL-Loads\Input-Files\TESTS_FM_SPECIMEN_RCD_NONULLS.txt".*/
...
/* 1dot1 */ 
            FIND FIRST TESTS_FM_SPECIMEN_RCD
                WHERE 
                        TESTS_FM_SPECIMEN_RCD.PatientID = DECIMAL(tt.PatientID) AND 
                        TESTS_FM_SPECIMEN_RCD.Lab_Sampleid = tt.Lab_Sampleid AND 
                        TESTS_FM_SPECIMEN_RCD.Lab_Sampleid_SeqNbr = DECIMAL(tt.Lab_Sampleid_SeqNbr)
                    EXCLUSIVE-LOCK NO-ERROR.                                                        
            
            IF AVAILABLE (TESTS_FM_SPECIMEN_RCD) THEN DO:                                               
                ...
                EXPORT STREAM outward DELIMITER ";"                                                 
                    ...
                    "Replaced TESTS_FM_SPECIMEN_RCD.".                 
                      
                DELETE  TESTS_FM_SPECIMEN_RCD  NO-ERROR.                                              
                 
            END.  /** IF AVAILABLE (TESTS_FM_SPECIMEN_RCD) **/    

/* 1dot1 */     
         
            CREATE TESTS_FM_SPECIMEN_RCD.
        
            ASSIGN     
                TESTS_FM_SPECIMEN_RCD.FM_Dental_Amalgams = tt.FM_Dental_Amalgams
                TESTS_FM_SPECIMEN_RCD.FM_Detoxification_agent = tt.FM_Detoxification_agent
                TESTS_FM_SPECIMEN_RCD.FM_Dosage = tt.FM_Dosage
                TESTS_FM_SPECIMEN_RCD.FM_Provocation = tt.FM_Provocation
                TESTS_FM_SPECIMEN_RCD.FM_Quantity = tt.FM_Quantity
                TESTS_FM_SPECIMEN_RCD.Lab_Sampleid = tt.Lab_Sampleid
                TESTS_FM_SPECIMEN_RCD.Lab_Sampleid_SeqNbr = tt.Lab_Sampleid_SeqNbr
                TESTS_FM_SPECIMEN_RCD.PatientID = tt.PatientID
                TESTS_FM_SPECIMEN_RCD.Progress_DateTime = DATE(tt.Progress_DateTime)
                TESTS_FM_SPECIMEN_RCD.Progress_Flag = tt.Progress_Flag
                TESTS_FM_SPECIMEN_RCD.Test_Abbv = tt.Test_Abbv
            .  
            
            IF TESTS_FM_SPECIMEN_RCD.Progress_DateTime = ? THEN 
                ASSIGN TESTS_FM_SPECIMEN_RCD.Progress_DateTime = TODAY. 
    
            EXPORT STREAM outward DELIMITER ";"                                                 
                    TESTS_FM_SPECIMEN_RCD.PatientID                                                
                    TESTS_FM_SPECIMEN_RCD.Lab_Sampleid                                  
                    TESTS_FM_SPECIMEN_RCD.Lab_Sampleid_SeqNbr                                                                     
                    "New TESTS_FM_SPECIMEN_RCD.".    
                                   
            RELEASE TESTS_FM_SPECIMEN_RCD.
