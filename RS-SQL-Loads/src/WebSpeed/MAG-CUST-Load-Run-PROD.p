/*------------------------------------------------------------------------
    ...
    Purpose     : Connect to the Magento System and extract the Magento Customer
                ...
                : then run the MAG_CUST-Load-PROD.p using the extracted 
    ...
    Version 2.1 - written by arold Luttrell, Sr. on 09/Oct/17.  Changed to use
...
{UTIL-set_propath-HHI_PROD-U.i}.  						/* 2dot1 */
...
ASSIGN cmdname = "P:/OpenEdge/PROD/Batch-Runs/magentoconnection.exe".                            /* 2dot1 */
ASSIGN cmdparam = " " + char-dte + " " + SEARCH("Input-Files/MAG-CUST-RCD-Extracted.txt").                   /* 2dot1 */
...
PUT UNFORMATTED "3 of 4. : MAG_CUST-Load-PROD.r" SKIP(1).                                       /* 2dot1 */

RUN VALUE(SEARCH("MAG_CUST-Load-PROD.r")).                                                      /* 2dot1 */
