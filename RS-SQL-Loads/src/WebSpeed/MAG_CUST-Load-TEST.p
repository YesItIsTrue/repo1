...
/*------------------------------------------------------------------------
    File        : MAG_CUST-Load-TEST.p                                              /* 3dot1 */
    Purpose     : Updates the MAGento CUSTomer ReCorD in the CUSTOM database.       /* 3dot1 */

    Description : Production program to read the selected (by date) 
                : Magento extracted .txt file and Update/Create the data 
                : in the MAG_CUST_RCD table in the CUSTOM database.
    ...
    <META NAME="LAST_UPDATED" CONTENT="17/Oct/17">
    ...
    Version 3.1 - written by Harold Luttrell, Sr. on 17/Oct/17.  Changed to use
...
/*{UTIL-set_propath-HHI_TEST-U.i}.                                                                    /* 3dot1 */*/
...
/* ***************************  E-Mail  Definitions  *************************** */
DEFINE VARIABLE From-Calling-Pgm            AS CHARACTER  FORMAT "x(118)"       NO-UNDO.    /* 3dot0 */
DEFINE VARIABLE e_to-whom                   AS CHARACTER  FORMAT "x(120)"       NO-UNDO.    /* 3dot0 */
DEFINE VARIABLE e_subject                   AS CHARACTER  FORMAT "x(80)"        NO-UNDO.    /* 3dot0 */
DEFINE VARIABLE e_message                   AS CHARACTER  FORMAT "x(4000)"      NO-UNDO.    /* 3dot0 */
DEFINE VARIABLE msg-attach-file-name        AS CHARACTER  FORMAT "x(4000)"      NO-UNDO.    /* 3dot0 */
...
ASSIGN From-Calling-Pgm = THIS-PROCEDURE:FILE-NAME.                                 /* 3dot0 */
ASSIGN e_to-whom = "".                                                              /* 3dot0 */
ASSIGN e_subject =  "-s Log Report from ".                                          /* 3dot0 */
ASSIGN e_message =  "-m Log Report attached from "                                  /* 3dot0 */
                                + h-pgm-name                                        /* 3dot0 */
                                + "\n"                                              /* 3dot0 */
                                + "End of message.".                                /* 3dot0 */
ASSIGN msg-attach-file-name = "".                                                   /* 3dot0 */
...
IF  IP_magento-id-BP = ""            OR
    IP_dr_tcp_id_scust_dr_id_col-F  = "doctor_tcp_id" THEN 
        NEXT mainloopie.

/*IF INTEGER(IP_magento-id-BP) > 8648 AND INTEGER(IP_magento-id-BP) < 10653 THEN*/
/*    DISPLAY  "IP_magento-id-BP = " INTEGER(IP_magento-id-BP)               */
/*             "col-F = " IP_dr_tcp_id_scust_dr_id_col-F  FORMAT "x(12)"     */
/*             "col-G = " IP_email-col-G                  FORMAT "x(32)" SKIP*/
/*             "col-S = " IP_billing_firstname-col-S      FORMAT "x(12)"     */
/*             "col-U = " IP_billing_lastname-col-U       FORMAT "x(18)"     */
/*             SKIP .                                                        */
/*                                                                           */
    IF limitdisplay > 0  AND  limitdisplay > Updated-cnt THEN
        DISPLAY  "IP_magento-id-BP = " IP_magento-id-BP SKIP.
    IF limitdisplay > 0  AND  limitdisplay > Updated-cnt THEN
        PAUSE.
...
/* 2dot0 */                         /* 2dot0 */                                 /* 2dot0 */
    ...
    IF  (IP_prefix-col-J = "Ms." OR IP_billing_prefix-col-R = "Ms." OR          /* 3dot0 */
         IP_prefix-col-J = "Mrs." OR IP_billing_prefix-col-R = "Mrs." OR        /* 3dot0 */
         IP_prefix-col-J = "Miss" OR IP_billing_prefix-col-R = "Miss")          /* 3dot0 */
        THEN                                                                    /* 3dot0 */
            ASSIGN IP_gender-col-BN = "Female".                                 /* 3dot0 */
                
    IF  (IP_prefix-col-J = "Mr." OR IP_billing_prefix-col-R = "Mr.")            /* 3dot0 */     
        THEN                                                                    /* 3dot0 */
            ASSIGN IP_gender-col-BN = "Male".                                   /* 3dot0 */
...
/* 3dot0 */                
                IF ERROR-STATUS:ERROR OR ERROR-STATUS:NUM-MESSAGES > 0 THEN DO:
                    ...
                    DISPLAY STREAM outward
                            Magento_Data.IP_magento-id-BP   FORMAT "x(10)"
                            Magento_Data.IP_firstname       FORMAT "x(15)"
                            Magento_Data.IP_lastname        FORMAT "x(20)"  
                            IP_email-col-G                  FORMAT "x(20)"
                            IP_dr_tcp_id_scust_dr_id_col-F  FORMAT "x(20)" SKIP
                            disp-msg                        FORMAT "x(40)" SKIP(1)
                        WITH FRAME outdetail WIDTH 132 DOWN.
                        DOWN WITH FRAME outdetail.
                END.                
                ELSE DO:
...
/* 3dot0 */                 
                IF ERROR-STATUS:ERROR OR ERROR-STATUS:NUM-MESSAGES > 0 THEN DO:
                    ...
                    DISPLAY STREAM outward
                            Magento_Data.IP_magento-id-BP   FORMAT "x(10)"
                            Magento_Data.IP_email-col-G     FORMAT "x(40)"
                            Magento_Data.IP_firstname       FORMAT "x(15)"
...
/* 3dot0 */                                                                             
                    ...
                    IF limitdisplay > 0  AND  limitdisplay > Created-cnt THEN                    
                        DISPLAY STREAM outward 
                                Magento_Data.IP_magento-id-BP   FORMAT "x(10)"                    
                                Magento_Data.IP_email-col-G     FORMAT "x(40)" 
                                Magento_Data.IP_firstname-col-K FORMAT "x(15)"
                                Magento_Data.IP_lastname-col-M  FORMAT "x(20)" 
                                disp-msg                        FORMAT "x(40)" SKIP  
                            WITH FRAME outdetail WIDTH 160 DOWN.
                            DOWN WITH FRAME outdetail. 
...
ASSIGN msg-attach-file-name = errlog.                                           /* 3dot0 */

RUN VALUE(SEARCH("Send-EMAIL-Message.r"))                                       /* 3dot0 */
            (From-Calling-Pgm,                                                  /* 3dot0 */
             e_to-whom,                                                         /* 3dot0 */  
             e_subject,                                                         /* 3dot0 */
             e_message,                                                         /* 3dot0 */
             msg-attach-file-name).                                             /* 3dot0 */

/* Invalid ISO State and/or Country Error Report. */                            /* 2dot0 */
IF  BadBState-cnt > 0 OR 
    BadBCountry-cnt > 0 OR 
    BadSState-cnt > 0 OR 
    BadSCountry-cnt > 0 THEN DO:                                                /* 2dot0 */
              
    ASSIGN msg-attach-file-name = ISOlog.                                       /* 3dot0 */

    ASSIGN e_subject =  "-s ISO Report from ".                                  /* 3dot0 */
    ASSIGN e_message =  "-m ISO Report attached from "                          /* 3dot0 */
                                    + h-pgm-name                                /* 3dot0 */
                                    + "\n"                                      /* 3dot0 */
                                    + "End of message.".                        /* 3dot0 */
    RUN VALUE(SEARCH("Send-EMAIL-Message.r"))                                   /* 3dot0 */
 ...
 END.  /* Invalid ISO State and/or Country Error Report. */
   
   
/*PROPATH = ORIGPROPATH.                                                          /* 3dot1 */*/
