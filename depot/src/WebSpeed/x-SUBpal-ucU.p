...
DO TRANSACTION:
    ...
    IF NOT i-update THEN /*Create a new record*/
        DO:
            IF (i-type = "Primary") THEN 
                DO: /*Change all other addresses linked to this people_id to secondary*/
                    IF AVAILABLE (pal_list) THEN 
                    DO:
                        FOR EACH pal_list WHERE pal_list.pal_people_id = i-people_id EXCLUSIVE-LOCK:
            ...
            ASSIGN 
                ...
                .      
        END.
    ELSE /*You can only update the type, since this is a many-to-many table*/
        DO:
            IF AVAILABLE pal_list THEN  
                ASSIGN 
                    o-update             = YES
