/*------------------------------------------------------------------------
    File        : HL7-Load-Main.p
    Purpose     : 

    Description : Load/Import/Extract the HL7 (XML) data from the 
                : General.fs_mstr and General.atn_det tables 
                : for processing and updates certin  
                : General Database and HHI Database table records.
...
/*DEFINE VARIABLE  h-ip-ans                   AS CHARACTER                        NO-UNDO.*/

/* ********************************************************************************************* */

DEFINE VARIABLE  i-flab-ID                  LIKE HHI.lab_mstr.lab_ID            NO-UNDO.
DEFINE VARIABLE  o-flab-name                LIKE HHI.lab_mstr.lab_name          NO-UNDO.
DEFINE VARIABLE  o-flab-error               AS LOGICAL INITIAL NO               NO-UNDO.

DEFINE VARIABLE  o-people-id                LIKE General.people_mstr.people_id  NO-UNDO.
DEFINE VARIABLE  o-people-load-error        AS LOGICAL INITIAL NO               NO-UNDO.

DEFINE VARIABLE  o-addr-id                  LIKE General.addr_mstr.addr_id      NO-UNDO.
DEFINE VARIABLE  o-addr-load-error          AS LOGICAL INITIAL NO               NO-UNDO.

DEFINE VARIABLE  o-TK-Mstr-id               LIKE HHI.TK_mstr.TK_ID              NO-UNDO.
DEFINE VARIABLE  o-Lab_Sample_ID            LIKE hhi.TK_mstr.TK_lab_sample_ID   NO-UNDO.
DEFINE VARIABLE  o-TK-Mstr-load-error       AS LOGICAL INITIAL NO               NO-UNDO.

DEFINE VARIABLE  o-TKR-det-id               LIKE HHI.TKR_det.TKR_ID             NO-UNDO. 
...
DEFINE VARIABLE i-att_file_id   LIKE general.att_files.att_file_id NO-UNDO.
DEFINE VARIABLE i-att_table     LIKE general.att_files.att_table NO-UNDO.
DEFINE VARIABLE i-att_field1    LIKE General.att_files.att_field1 NO-UNDO.
DEFINE VARIABLE i-att_value1    LIKE General.att_files.att_value1 NO-UNDO.
DEFINE VARIABLE i-att_field2    LIKE General.att_files.att_field2 NO-UNDO.
DEFINE VARIABLE i-att_value2    LIKE General.att_files.att_value2 NO-UNDO.
DEFINE VARIABLE i-att_field3    LIKE General.att_files.att_field3 NO-UNDO.
DEFINE VARIABLE i-att_value3    LIKE General.att_files.att_value3 NO-UNDO.
DEFINE VARIABLE i-att_field4    LIKE General.att_files.att_field4 NO-UNDO.
DEFINE VARIABLE i-att_value4    LIKE General.att_files.att_value4 NO-UNDO.
DEFINE VARIABLE i-att_field5    LIKE General.att_files.att_field5 NO-UNDO.
DEFINE VARIABLE i-att_value5    LIKE General.att_files.att_value5 NO-UNDO.
DEFINE VARIABLE i-att_category  LIKE general.att_files.att_category NO-UNDO.
DEFINE VARIABLE i-att_filepath  LIKE General.att_files.att_filepath NO-UNDO.
DEFINE VARIABLE i-att_filename  LIKE general.att_files.att_filename NO-UNDO.
DEFINE VARIABLE i-att_filedesc  LIKE general.all_files.att_filedesc NO-UNDO.
DEFINE VARIABLE i-att_major_version LIKE General.att_files.att_major_version NO-UNDO.
DEFINE VARIABLE i-att_minor_version LIKE General.att_files.att_minor_version NO-UNDO.

DEFINE VARIABLE o-att_file_id       LIKE general.att_files.att_file_id      NO-UNDO.
...
DEFINE VARIABLE h-TK_ID                 LIKE HHI.TK_mstr.TK_ID              NO-UNDO. 
DEFINE VARIABLE h-TK_test_seq           AS CHARACTER                        NO-UNDO. 
DEFINE VARIABLE h-Lab_Sample_ID         LIKE HHI.TK_mstr.TK_lab_sample_ID   NO-UNDO.
...
/* Store off the PROPATH parameters to be re-set at the end of this program executation. */                    
ASSIGN
...
/* if set to YES - program will generate dumps/list the different data files to assist in
...
DEFINE VARIABLE loadRpt AS CHARACTER 
    INITIAL "C:\PROGRESS\WRK\HL7-Load-Main-Rpt.txt" NO-UNDO.
...
/* Add the following HL7 r-code paths to the front of the PROPATH. */
 
IF drive_letter = "P" THEN DO:
    PROPATH =   "P:\OpenEdge\WRK\HL7\rcode\," +
                "P:\OpenEdge\WRK\depot\rcode\," +
                "P:\OpenEdge\WRK\DataHub\rcode\," +
                PROPATH.
END.

ELSE IF drive_letter = "C" THEN DO:
    PROPATH =   "C:\OpenEdge\Batch-Runs\," +
                "C:\OpenEdge\Workspace\HL7\rcode\," +
                "C:\OpenEdge\Workspace\HL7\src\," +
                "C:\OpenEdge\Workspace\depot\rcode\," +
                "C:\OpenEdge\Workspace\DataHub\rcode\," +
                "C:\OpenEdge\Workspace\HL7\," +
                PROPATH.
END.
...
/*                                                               */
/*INPUT FROM "C:\OpenEdge\Batch-Runs\HL7-YES-Ans.txt" NO-CONVERT.*/
/*IMPORT h-ip-ans.                                               */
/*INPUT CLOSE.                                                   */
...
/* ***************************************************************************** *  
 * Extract the input HL7 data from the General.fs_mstr & General.atn_det tables. * 
 * Section 1.                                                                    * 
...
FOR EACH General.fs_mstr    WHERE  fs_mstr.fs_deleted = ?   
            ...
            EXCLUSIVE-LOCK,
    EACH General.atn_det    WHERE  atn_det.atn_deleted = ? 
...
/* Start of a new HL7 input. Space out the variable ip-Table-Name.  */
    ...
    IF  TRIM(General.atn_det.atn_name) =  "root"    THEN DO: 
        
             ASSIGN  ip-Table-Name      = "". 
             
             NEXT FirstLoopie.
             
        END.  /* IF  TRIM(General.atn_det.atn_name) =  "root"  */ 
...
/* increment the c-tests-processed counter by 1 and 
    ...
    IF  (General.fs_mstr.fs_parent_level = 1)   AND 
        (General.atn_det.atn_node_level  = 2)   AND 
        TRIM(General.atn_det.atn_name)  = "TK_test_seq" AND 
        TRIM(General.atn_det.atn_value) = "" THEN DO: 
    ...
    END. /* IF (General.fs_mstr.fs_parent_level = 1 & = 2) */
...
/*          UTEE input test files have 2 or more tests results within their files. */
    ...
    IF  (General.fs_mstr.fs_parent_level = 1)   AND 
        ...
        TRIM(General.atn_det.atn_value) = "" THEN DO: 
            ...
            ASSIGN  utee-fs-parent-level   = General.atn_det.atn_node_level
                    c-tests-processed      = (c-tests-processed + 1).
    ...
    IF  utee-fs-parent-level   > 0 AND 
        utee-fs-parent-level   = General.fs_mstr.fs_parent_level THEN DO: 
                                           
           ASSIGN  ip-Table-Name = TRIM(General.atn_det.atn_name).
           
           NEXT FirstLoopie.
           
    END. /* IF  utee-fs-parent-level   > 0 */
    
/* get the input table name. */
   
    IF  h-ip-table-na  = "SNP_ID"                   AND 
        ip-Table-Name  = "trh_hist"                 AND 
        TRIM(General.atn_det.atn_name) = "tk_mstr"  THEN 
        
            ASSIGN  ip-Table-Name   = "SNP_ID"
                    h-ip-table-na   = "".

    IF  ip-Table-Name                               = "SNP_ID"  AND   
        SUBSTRING(General.atn_det.atn_name, 1, 3)   =  "tk_"    THEN
        
            ASSIGN  ip-Table-Name   =  "SNP_ID".
            
    ELSE       
    
    IF  (General.fs_mstr.fs_parent_level = 2)   AND 
         TRIM(General.atn_det.atn_value) = ""   THEN DO:
    ...
    END.  /* IF  atn_det.atn_value = "" */
               
/* Store the SNP FAKE table name.  */
                 
    IF  TRIM(General.atn_det.atn_name)    = "SNP_ID"    AND
        TRIM(General.atn_det.atn_value)   = ""          AND
        General.atn_det.atn_file_ID       = General.fs_mstr.fs_file_ID THEN DO:
    ...
    IF  (TRIM(General.atn_det.atn_name) = "coll_begin"   OR 
         TRIM(General.atn_det.atn_name) = "coll_end"   ) AND 
         TRIM(General.atn_det.atn_value) = ""            THEN DO:   
     ...
     * ***************************************************** */    
                ...
                ASSIGN  XML_TT_1_Data.TT-1-Seq-Nbr        = Temp-1-Seq-Nbr                 
                        XML_TT_1_Data.TT-1-File-ID        = General.fs_mstr.fs_file_id
                        XML_TT_1_Data.TT-1-parent-level   = General.fs_mstr.fs_parent_level
                        XML_TT_1_Data.TT-1-p-child-node-level    = General.fs_mstr.fs_child_level
                        
                        XML_TT_1_Data.TT-1-Table-Name     = TRIM(ip-Table-Name) 
                        XML_TT_1_Data.TT-1-Field-Name     = TRIM(General.atn_det.atn_name)
                        XML_TT_1_Data.TT-1-Field-Value    = TRIM(General.atn_det.atn_name).       /* force the name into the value field for dates. */
    ...
    IF  SNP-to-trh-hist                     = 0         AND               
        ...
        (TRIM(General.atn_det.atn_name)     = "year"    OR 
         TRIM(General.atn_det.atn_name)     = "month"   OR  
         TRIM(General.atn_det.atn_name)     = "day" )   AND 
         TRIM(General.atn_det.atn_value)    = ""        THEN DO:   
 ...
 * *********************************************************** */    
    ...
    IF  TRIM(General.atn_det.atn_value) <> "" THEN DO: 
        ...
        ASSIGN  XML_TT_1_Data.TT-1-Seq-Nbr        = Temp-1-Seq-Nbr         
                ...
                XML_TT_1_Data.TT-1-Table-Name     = TRIM(ip-Table-Name) 
                XML_TT_1_Data.TT-1-Field-Name     = TRIM(General.atn_det.atn_name)
                XML_TT_1_Data.TT-1-Field-Value    = TRIM(General.atn_det.atn_value). 
 
        /* Flag the End of each tests with the incremented counter. */
        IF  SUBSTRING(General.atn_det.atn_name, 1, 3) = "xsi" THEN DO:  
...
END.  /** FirstLoopie: of 4ea. fs_mstr, etc. **/
...
IF  TT-1-Seq-Nbr = 1 THEN 
    PUT STREAM outwardTTD   "****  Trying to Call: SUBlabid-findR.r  ****" SKIP.
...
/* increment the TT files Seq-Nbr FOR every input test_mstr data line. */
    ...
    END.  /* IF TT-1-Table-Name = "test_mstr" */
...
/*        IF  XML_TT_1_Data.TT-1-Field-Name   = "people_dob" THEN DO:*/
        ...
        END.  /* XML_TT_1_Data.TT-1-Field-Name   = "SSN" */       
...
{XML-DB-Std-ERROR-Code-2.i}.                                                                             
    ...
    END.  /* IF TT-1-Table-Name = "people_mstr" */
...
{XML-DB-Std-ERROR-Code-2.i}.  
    ...
    END.  /* IF TT-1-Table-Name = "addr_mstr" */
...
/* get the BIOMED collection begin date. */
        ...
        END. /* IF XML_TT_TK_Mstr_Data.TT-TK_Mstr_coll_begin_day = "" */
...
/* get the BIOMED collection end date. */ 
        ...
        END.  /* IF  XML_TT_TK_Mstr_Data.TT-TK_Mstr_coll_end_day = "" */
...
{XML-DB-Std-ERROR-Code-2.i}.   
    ...
    END.  /* IF XML_TT_1_Data.TT-1-Table-Name = "tk_mstr" */
...
END. /*  PeopleLoopie: FOR EACH XML_TT_People_Data  */
...
END. /*  AddressLoopie: XML_TT_Address_Data  */
...
END. /* FOR EACH XML_TT_PeopID_Basic_Data */
...
/*IF  ITDisplay = YES THEN*/
    FOR EACH XML_TT_PeopID_Basic_Data   NO-LOCK BREAK BY  TT_PeopID_Seq_Nbr_only:     
        ...
        PUT STREAM PeopBasic
              ...
              XML_TT_PeopID_Basic_Data.TT_PeopID_Discrep_Table      FORMAT "x(10)"   " "
...
/*DISPLAY "calling 1: SUBtrh-HL7-U.r" SKIP.*/
    RUN VALUE (SEARCH("SUBtrh-HL7-U.r"))
            ...
            IF ERROR-STATUS:ERROR OR ERROR-STATUS:NUM-MESSAGES > 0 THEN DO:
               DISPLAY ERROR-STATUS:NUM-MESSAGES " Run error8 - RUN VALUE(SEARCH(." SKIP.      
                  DO ix = 1 TO ERROR-STATUS:NUM-MESSAGES: 
                      DISPLAY ERROR-STATUS:GET-NUMBER(ix) ERROR-STATUS:GET-MESSAGE(ix) SKIP.
                  END.
            END.
...
/* Delete the .txt file from last run.
    ...
    we can get the DATE the file was received into the server. */
        ...
        OS-COMMAND  SILENT    
            VALUE ("DEL C:\Progress\WRK\HL7-dirlist.txt").
            
        OS-stat = OS-ERROR. 
        IF  OS-stat <> 0 THEN DISPLAY "OS-ERROR DEL = " OS-stat SKIP.
           
        OS-COMMAND  SILENT    
            VALUE ("DIR " + h_from_path  + " >> C:\Progress\WRK\HL7-dirlist.txt"). 
...
The data file date should be in Field-5, unless someone changes this code!*/  
        ...
        INPUT FROM "C:\Progress\WRK\HL7-dirlist.txt".
...
END. /* trh_hist-Loopie: FOR EACH XML_TT_PeopID_Basic_Data */
...
/*NEW  FIND ATT-FILES USING the new fs_mstr-id sub-routine here.*/
    DO  ithree = 1 TO 3: 
        IF  ithree = 1 THEN   
            FIND  FIRST att_files WHERE  att_table = "fs_mstr" AND att_field1 = "fs_file_ID" AND 
                                         att_value1 = STRING(XML_TT_PeopID_Basic_Data.TT_PeopID_fs_atn_file_id) AND 
                                         att_filepath = "C:\apps\HL7\Processed\XML\successful\"    
                                EXCLUSIVE-LOCK NO-ERROR.  
                               
        ELSE  IF  ithree = 2 THEN 
                  FIND  NEXT att_files WHERE  att_table = "fs_mstr" AND att_field1 = "fs_file_ID" AND 
                                              att_value1 = STRING(XML_TT_PeopID_Basic_Data.TT_PeopID_fs_atn_file_id) AND 
                                         att_filepath = "C:\apps\HL7\Processed\EDI\successful\"     
                                    EXCLUSIVE-LOCK NO-ERROR. 
                              
        ELSE  IF  ithree = 3 THEN 
                  FIND  LAST att_files WHERE  att_table = "fs_mstr" AND att_field1 = "fs_file_ID" AND 
                                              att_value1 = STRING(XML_TT_PeopID_Basic_Data.TT_PeopID_fs_atn_file_id) AND 
                                         att_filepath = "C:\apps\HL7\Processed\PDF\successful\"    
                                    EXCLUSIVE-LOCK NO-ERROR.  
...
/* Get the PATH and it's FILE NAME to MOVE the files FROM and then use the 
    OD-COMMAND's to CREATE the people directory and then MOVE the files TO for the people_id folder. */
            ...
            ASSIGN  h_file_name = att_filename
                    ...
                    h_HTML_prefix = "file:\\hhi-dc-4.hhi.local\"
...
/* NOTE:  The following sequence is what RJ sent to me in the first test data file.
          If he changes the file sequences of what he stores, then the 3 if statements need to be changed also. */
            ...
            ASSIGN  i-att_table     = "TK_mstr"       
                    ...
                    i-att_filepath  = h_HTML_prefix + SUBSTRING(dir_name, 4, len) + "\".                                  
/* DISPLAY "calling : SUBatt-ucU.r" SKIP.*/
    ...
    END. /* DO  ithree = 1 TO 3: */
              
END.  /*  att_files-Loopie:  FOR EACH XML_TT_PeopID_Basic_Data */                    
...
RUN VALUE(SEARCH("HL7-Load-Append-Rpts.r")).
