...
/*------------------------------------------------------------------------
    ...
    Purpose     : Load/Update the TKR_det data from the XML data. 
...
DEFINE OUTPUT PARAMETER o-TKR-det-id            LIKE TKR_det.TKR_ID             NO-UNDO.
...
ASSIGN loadRPT = TT-Logs-Rpts-Path-Folder + "TestsDetails-Load-Rpt.txt".
...
/*        INITIAL "C:\PROGRESS\WRK\XML-SUB-TKR_det-Dump.txt"                      NO-UNDO.*/
    DEFINE VARIABLE dumpRptR AS CHARACTER NO-UNDO. 
    ASSIGN dumpRptR = TT-Logs-Rpts-Path-Folder + "XML-SUB-TKR_det-Dump.txt".
...
FOR EACH XML_TT_tkr_det_Data NO-LOCK        BREAK BY TT-tkr_det_fs_atn_file_ID 
    ...
    IF  AVAILABLE (people_mstr) THEN 
        ...
        ASSIGN  v-people-firstname      = people_mstr.people_firstname 
                v-people-midname        = people_mstr.people_midname
                v-people-lastname       = people_mstr.people_lastname
                v-people-dob            = people_mstr.people_DOB.    
    ...
    ASSIGN  tkDrecordsprocessed = (tkDrecordsprocessed + 1)
            ...
            h-uom       = ""
            h-ref       = "".  
/* space/blank out the table of Info-Msg's */ 
    DO  xy1 = 1 TO 11 BY 1 :
            ASSIGN  Info-Msg[xy1] = "".  
    END. 
         ...
         THEN DO: 
            ...
            IF  AVAILABLE (XML_TT_PeopID_Basic_Data) THEN   
                ASSIGN  XML_TT_PeopID_Basic_Data.TT_PeopID_ERROR_Flag  = "ERROR"
                        XML_TT_PeopID_Basic_Data.TT_PeopID_ERROR_Desc  = "TKR-ID or TKR_item is blank or TKR_ID_Seq is zero (0).".  
...
/* get the TK_Mstr.TK_test_type from the  XML_TT_TK_Mstr_Data record for each tests to
    determine if it is a brother record: FM, HE, UTEE or MPA. */
        ...
        IF  AVAILABLE (XML_TT_TK_Mstr_Data)  THEN DO: 
          ...
          IF  XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "FM"     OR  
              XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "HE"     OR    
              XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "UTEE"   OR  
              XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "UTM"    OR 
              XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "MPA"    THEN DO: 
...
/*   > 999   */             IF  XML_TT_tkr_det_Data.TT-tkr_det_TKR_item     = "H-ZnCd ratio" AND 
            ...
            ELSE        
                IF (XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type    = "UTEE" OR 
                    XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type    = "UTM")  AND  
...
/* See if the BFM_mstr was deleted and if so then UN-DELETE it by setting the deleted flag to ?. */ 
                            ...
                            END.  /** of else do --- create BMPA_mstr **/
...
/* Change the 2-char data (position 3 & 4) in the tkr_item FIELD AND make it the LONG NAME > e.g. U-Al/Creat:  Al = Aluminun, etc */     
        ...
        /* 2nd look for marker_item. */ 
            ...
            END. /* IF NOT AVAILABLE 2nd (marker_list)  */
            ...
            END.  /* IF NOT AVAILABLE (marker_list) */
        
            ELSE DO:
        
                ASSIGN  XML_TT_tkr_det_Data.TT-tkr_det_TKR_item = marker_list.marker_desc.
        
            END.  /* ELSE DO: */
/* End of input element-name/marker_item validation. */    

          END.  /*  IF  XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "FM"     OR  
                        XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "HE"     OR    
                        XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "UTEE"   OR  
                        XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "UTM"    OR 
                        XML_TT_TK_Mstr_Data.TT-TK_Mstr_TK_test_type = "MPA"   */                         
        END. /* IF AVAILABLE (XML_TT_TK_Mstr_Data) */


/* Process_NON_Brother_Rcds */
        ...
        IF  NOT AVAILABLE (TKR_det) THEN DO: 
            ...
            RUN VALUE(SEARCH("SUBtkrdet-ucU.r"))        
             ...
             ELSE DO:   
                ...
                ASSIGN  XML_TT_PeopID_Basic_Data.TT_PeopID_ERROR_Flag = "ERROR"
                        XML_TT_PeopID_Basic_Data.TT_PeopID_ERROR_Desc = "TKR-det record did not get created.".
            ...
            i-Admin-Update-OverRyde = YES   THEN DO: 
                ...
                RUN VALUE(SEARCH("SUBtkrdet-ucU.r"))        
                     ...
                     ELSE DO:   
                        ...
                        ASSIGN  XML_TT_PeopID_Basic_Data.TT_PeopID_ERROR_Flag = "ERROR"
                                XML_TT_PeopID_Basic_Data.TT_PeopID_ERROR_Desc = "TKR-det record did not get updated in OverRyde option.".
...
PUT STREAM outwardTKR SKIP(1)
    ...
    Element_Symbol_Error_kount FORMAT ">,>>9"       "  :TKR_item Element-Symbol not found in the marker_list." SKIP(2).
